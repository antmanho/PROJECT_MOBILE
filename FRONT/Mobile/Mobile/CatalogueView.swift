...
// Structure pour décoder la réponse du backend
...
struct CatalogueView: View {
    ...
    // Base URL pour les images (identique à celle de votre back)
    let baseImageURL = "http://localhost:3000"
    
    // Grille adaptative avec une largeur minimale réduite pour afficher 2 cartes par ligne
    let columns = [GridItem(.adaptive(minimum: 160), spacing: 10)]
    
    // Filtrer les jeux en fonction du champ de recherche
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 0) {
                // En-tête avec titre, champ de recherche et boutons
                VStack(spacing: 10) {
                    Text("CATALOGUE")
                        .font(.largeTitle)
                        .bold()
                    ...
                    HStack(spacing: 10) {
                        ...
                        // Bouton de recherche (image "rechercher")
                        Button(action: {
                        ...
                        // Bouton de réglage (image "reglage")
                        Button(action: {
                ...
                ScrollView {
                    LazyVGrid(columns: columns, spacing: 10) {
                        ForEach(filteredGames) { game in
                            NavigationLink(destination: DetailArticleView(game: game)) {
                                VStack(spacing: 0) {
                                    // Nom du jeu
                                    Text(game.nomJeu)
                                        .font(.headline)
                                ...
                                .frame(width: 160) // Réduction de la largeur des cartes
                                .background(Color.white)
                                .cornerRadius(10)
    ...
    // Fonction pour récupérer le catalogue des jeux en vente depuis le back
    func fetchCatalogue() {
